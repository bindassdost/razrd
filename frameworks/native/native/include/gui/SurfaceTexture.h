...
private:
    ...
    // *** MediaTek ******************************************************* //
private:
    // for buffer format conversion
    bool mAuxSlotConvert;
    bool mAuxSlotDirty;

    // aux buffer slot for format convert
    // for video or camera data input not match texture foramt required by G3D
    EGLSlot mAuxSlot[2];
    EGLSlot *mBackAuxSlot;
    EGLSlot *mFrontAuxSlot;

    // for drawing debug line
    bool mLineAux;

    status_t freeAuxSlotLocked(EGLSlot &s);

    // backup function when no hw conversion available
    status_t swConversionLocked(EGLSlot &src, EGLSlot &dst);

public:
    // get connected api type, for buffer data conversion condition (aux and hwc)
    int getConnectedApi() const { return mBufferQueue->getConnectedApi(); }
