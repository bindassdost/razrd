...
status_t BufferQueue::dequeueBuffer(int *outBuf, uint32_t w, uint32_t h,
    ...
    // [mtk04189] give a warning if dequeueBuffer() in a disconnected state
    //------------------------------------------------------------------------
            ...
            for (int i = 0; i < mBufferCount; i++) {
                ...
                if (false) {
                    ...
                    if (state == BufferSlot::FREE) {
                         ...
                         * buffers in flight.
                         */
                        // [MTK] 20120814: fix google stupid bug since
                        //                 found is -1 at first time
                        if (found < 0) {
                            foundSync = i;
                            found = i;
        ...
        *outBuf = found;
            ...
            ((uint32_t(buffer->usage) & usage) != usage))
        {
            ...
            if (graphicBuffer == 0) {
                ...
                // [Yi-Cheng Chen] display graphic buffer info after alloc
                //------------------------------------------------------------
                if (buffer != NULL) {
                    ST_LOGI("createGraphicBuffer before(%p, handle=%p, w=%d, h=%d, f=%d) after(%p, handle=%p, w=%d, h=%d, f=%d)",
                        buffer.get(), buffer->handle, buffer->width, buffer->height, buffer->format,
                        graphicBuffer.get(), graphicBuffer->handle, graphicBuffer->width, graphicBuffer->height, graphicBuffer->format);
                }
                else {
                    ST_LOGI("createGraphicBuffer before(NULL) after(%p, handle=%p, w=%d, h=%d, f=%d)",
                        graphicBuffer.get(), graphicBuffer->handle, graphicBuffer->width, graphicBuffer->height, graphicBuffer->format);
                }
                //------------------------------------------------------------
            }
        ...
        // [MTK] 20120904: move debug message into mutex protection region
...
status_t BufferQueue::queueBuffer(int buf,
    ...
    // [mtk04189] give a warning if queueBuffer() in a disconnected state
    //------------------------------------------------------------------------
    if (NO_CONNECTED_API == mConnectedApi) {
    ...
    { // scope for the lock
        ...
        // [mtk] if queue not empty, means consumer is slower than producer
        // * in sync mode, may cause lag (but size 1 should be OK for triple buffer)
        // * in async mode, frame drop
        //-------------------------------------------------------------------------------
        if (true == mSynchronousMode) {
            if (1 < mQueue.size()) {
                XLOGI("[BQ::queueBuffer] %s(%p), api=%d, queued=%d (lag)",
                    mConsumerName.string(), this, mConnectedApi, mQueue.size());
            }
        } else {
            if (0 < mQueue.size()) {
                XLOGI("[BQ::queueBuffer] %s(%p), api=%d, queued=%d (drop frame)",
                    mConsumerName.string(), this, mConnectedApi, mQueue.size());
            }
        }
        //-------------------------------------------------------------------------------
