      ...
      mInitCheck(NO_ERROR), mIndex(-1)
{
    ...
    XLOGI("create GraphicBuffer by existing handle (w:%d h%d f:%d) owner(%d)", stride, height, format, mOwner);
    if (mOwner == ownHandle) {
        mapBuffer();
    }
}
      ...
      mInitCheck(NO_ERROR), mIndex(-1), mWrappedBuffer(buffer)
{
    ...
    XLOGI("create GraphicBuffer by existing native window buffer (w:%d h%d f:%d) owner(%d)", stride, height, format, mOwner);
    if (mOwner == ownHandle) {
        mapBuffer();
    }
}
