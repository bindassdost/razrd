...
void LayerBase::initStates(uint32_t w, uint32_t h, uint32_t flags)
{
    ...
    // [mtk03712] init for S3D
    //------------------------------------------------------------------------
    mCurrentState.flags |= ISurfaceComposer::eLayerUnknown;
    mCurrentState.requested_flags = ISurfaceComposer::eLayerInvalid;
    mCurrentState.s3d_flags = ISurfaceComposer::eLayer2D;
    //------------------------------------------------------------------------
...
void LayerBase::commitTransaction() {
    ...
    // [mtk03712] dump state result after transaction committed
    //------------------------------------------------------------------------
...
bool LayerBase::setFlags(uint8_t flags, uint8_t mask) {
    ...
    // [mtk03712] for s3d flag only
    //------------------------------------------------------------------------
    if (ISurfaceComposer::eLayerS3DMask == mask) {
        State &cs = mCurrentState;
        cs.requested_flags = (flags & mask);
        XLOGI("[%s] %s requested:0x%02x", __func__, mName.string(), cs.requested_flags);
        return true;
    }
    //------------------------------------------------------------------------
...
void LayerBase::setPerFrameData(hwc_layer_t* hwcl) {
    ...
    // *** MediaTek ******************************************************* //
    hwcl->mtkData.buffer = NULL;
    hwcl->mtkData.connectApi = 0;
    // ******************************************************************** //
}
...
void LayerBase::drawWithOpenGL(const Region& clip) const
{
    ...
    // [mtk03712] for S3D surface
    //------------------------------------------------------------------------
    adjustTexCoord((void*)texCoords);
    //------------------------------------------------------------------------    
