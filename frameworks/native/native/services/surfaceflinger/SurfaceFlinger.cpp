...
void SurfaceFlinger::init()
{
    ...
    mFeatureState.mTVOUT = true;
#else
    mFeatureState.mTVOUT = false;
#endif

#ifdef MTK_HDMI_SUPPORT
    mFeatureState.mHDMI = true;
#else
    mFeatureState.mHDMI = false;
#endif

#ifdef MTK_S3D_SUPPORT
    mFeatureState.mS3D = true;
#else
    mFeatureState.mS3D = false;
#endif
...
status_t SurfaceFlinger::readyToRun()
{
    ...
    // [added by Ryan] for SW watch dog
    mWatchDog = new SFWatchDog(this);
...
void SurfaceFlinger::startBootAnim() {
    ...
    // [MTK] dynamic disable/enable boot animation
...
//        case MessageQueue::INVALIDATE: {
            ...
            // [mtk03712] check and set to S3D composing
            handleComposingS3DSetting();
...
//            }
            ...
            if (CC_LIKELY(hw.canDraw())) {
                ...
                if (0 != mPropertiesState.mDelayTime) {
                    XLOGI("SurfaceFlinger slow motion timer: %d ms", mPropertiesState.mDelayTime);
...
void SurfaceFlinger::handleTransactionLocked(uint32_t transactionFlags)
{
    ...
    if (transactionFlags & eTransactionNeeded) {
        ...
        if (mLayersRemoved) {
            ...
            // [mtk04189] lazy swap case 1: Draw and swap if layer removed
    ...
    const LayerVector& currentLayers, Region& dirtyRegion, Region& opaqueRegion)
{
    ...
    while (i--) {
        ...
        // [mtk04189] lazy swap case 2: Draw and swap if layer content updated (by transaction) in drawing state
...
void SurfaceFlinger::handleRepaint()
{
...
#if 1
    // [mtk04189] do not render transparent/wormhole/dirty regions if unnecessary
    //if (mLayersSwapRequired) {
        // [mtk03712] mtk S3D screen composing
        composeSurfacesS3D(mDirtyRegion);
    //}
#else
    ...
    // [added by Ryan] for drawing debug line
    //------------------------------------------------------------------------
...
void SurfaceFlinger::setupHardwareComposer()
{
    ...
    // [MTK] 20120726: reserve previous count for layers handled by gpu
    ...
    // *** MediaTek ******************************************************* //
    // [mtk03712] debug log
    if (true == mPropertiesState.mLogRepaint && NO_ERROR == err) {
        XLOGD("@ HWComposer::prepare success (HWC_FRAMEBUFFER:%d, HWC_OVERLAY:%d)",
              hwc.getLayerCount(HWC_FRAMEBUFFER), hwc.getLayerCount(HWC_OVERLAY));
    }

    // [mtk04189] do not render transparent region if unnecessary
    checkLayersSwapRequired(fbLayerCount);
    mLayersSwapRequired = (true == mPropertiesState.mBusySwap) ? true : mLayersSwapRequired;
    
    if (true == mPropertiesState.mLogRepaint) {
        XLOGD("@ mLayersSwapRequired:%d", mLayersSwapRequired);
    }
    // ******************************************************************** //
}
...
void SurfaceFlinger::screenAcquired() {
    ...
    // [mtk04189] lazy swap case 6: Draw and swap if screen is about to return
    mLayersSwapRequired = true;

    // [add by Ryan] SF watch dog
    if (NULL != mWatchDog.get()) {
        mWatchDog->screenAcquired();
    }
    ...
    // [mtk03712] pass IPO a signal for the black screen sync issue
    //------------------------------------------------------------------------
    usleep(16667);
...
status_t SurfaceFlinger::dump(int fd, const Vector<String16>& args)
{
    ...
    // [added by Ryan] for run-time enable property
...
void SurfaceFlinger::repaintEverything() {
    // [mtk04189] lazy swap case 7: Draw and swap if region is invalidated
    mLayersSwapRequired = true;
        ...
        GLuint* textureName, GLfloat* uOut, GLfloat* vOut)
{
    ...
    // [mtk03712] temp force change to 2D composing mode of S3D
    //------------------------------------------------------------------------
    uint32_t op = mDrawingState.composingPhase;
    mDrawingState.composingPhase = State::eComposing2D;
    ...
    // [mtk03712] change mode back
    mDrawingState.composingPhase = op;
        ...
        uint32_t minLayerZ, uint32_t maxLayerZ)
{
    ...
    // [mtk03712] solution by mtk00849, for hwrotation
    //------------------------------------------------------------------------
    ...
    if (status == GL_FRAMEBUFFER_COMPLETE_OES) {
        ...
        if (glGetError() != GL_NO_ERROR) {
            ...
            if (ptr) {
                        ...
                        if (glGetError() == GL_NO_ERROR) {
                            ...
                            switch (displayOrientation) {
                                ...
                                case 3:
                                    ...
                                    break;
                                default:
                                    XLOGW("[%s] unknown orientation: %d, regard as no rotation",
                                        __func__, displayOrientation);
                                    dx = sw;
                                    dy = sh;
                                    trg_w = sw;
                                    trg_h = sh;
                ...
                // [mkt03712] dump screenshot result, debug only
                //------------------------------------------------------------
                if (true == mPropertiesState.mDumpScreenShot) {
                    static int cnt = 0;
                    char             fname[128];
                    SkBitmap         b;
                    SkBitmap::Config c = SkBitmap::kARGB_8888_Config;
                    sprintf(fname, "/data/%d.png", 100000 + cnt);
                    b.setConfig(c, sw, sh);
                    b.setPixels(ptr);
                    SkImageEncoder::EncodeFile(
                        fname, b, SkImageEncoder::kPNG_Type, SkImageEncoder::kDefaultQuality);
                    XLOGE("[%s] dump %s done", __func__, fname);
                    cnt += 1;
                }
                //------------------------------------------------------------
...
void GraphicPlane::setDisplayHardware(DisplayHardware *hw)
{
    ...
    // [mtk] save orientation in GraphicPlane for hwrotation
