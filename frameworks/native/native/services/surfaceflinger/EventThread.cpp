...
{
    // [MTK] 20120906: workaround to avoud deadlock
    tid = -1;
}
...
status_t EventThread::unregisterDisplayEventConnection(
    ...
    if (tid != gettid()) {
        Mutex::Autolock _l(mLock);
        XLOGD("Disconnecting from (%d)", gettid());
        mDisplayEventConnections.remove(connection);
        mCondition.broadcast();
    } else {
        XLOGW("Disconnecting from EventThread (%d)", tid);
        mDisplayEventConnections.remove(connection);
    }
    return NO_ERROR;
}
...
bool EventThread::threadLoop() {
    ...
    // [MTK] 20120905: log EventThread cycle
    //XLOGD("[%s] threadLoop prepare to get mutex", __func__);
    ...
    // [MTK] 20120905: log EventThread cycle
    //XLOGD("[%s] threadLoop release mutex", __func__);
...
void EventThread::dump(String8& result, char* buffer, size_t SIZE) const {
    ...
    for (size_t i=0 ; i<mDisplayEventConnections.size() ; i++) {
        sp<Connection> connection =
                mDisplayEventConnections.itemAt(i).promote();
        result.appendFormat("    %p: count=%d, pid=%d\n",
                connection.get(), connection!=NULL ? connection->count : 0,
                connection!=NULL ? connection->pid : 0);
    }
}
...
{
    // [MTK] 20120905: reserve client pid infomation
    pid = IPCThreadState::self()->getCallingPid();
    XLOGI("EventThread Client Pid (%d) created", pid);
}

EventThread::Connection::~Connection() {
    // [MTK] 20120905: print client pid infomation
    XLOGI("EventThread Client Pid (%d) disconnecting by (%d)...", pid, getpid());
...
void EventThread::Connection::onFirstRef() {
    // [MTK] 20120905: print client pid infomation
    XLOGI("EventThread Client Pid (%d) connecting by (%d)...", pid, getpid());
    // NOTE: mEventThread doesn't hold a strong reference on us
    mEventThread->registerDisplayEventConnection(this);
    XLOGI("EventThread Client Pid (%d) connected by (%d)", pid, getpid());
}
