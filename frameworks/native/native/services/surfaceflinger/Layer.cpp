...
#else
    // [MTK] 20120818: read board prop to check if need to disable triple buffer
...
void Layer::setGeometry(hwc_layer_t* hwcl)
{
    ...
    const State& s(drawingState());
    if ((s.alpha < 0xFF) || (true == isProtected())) {    // [mtk03712] skip from HWC when protected
        hwcl->flags = HWC_SKIP_LAYER;
    }
    ...
    // *** MediaTek ******************************************************* //
    hwcl->transform = finalTransform;
    hwcl->mtkData.identity = getIdentity();
    hwcl->mtkData.fillColor.a = s.alpha;
    float *m = hwcl->mtkData.transformMatrix;
...
void Layer::setPerFrameData(hwc_layer_t* hwcl) {
    ...
    // *** MediaTek ******************************************************* //
    hwcl->mtkData.buffer = buffer;
    hwcl->mtkData.connectApi = mSurfaceTexture->getConnectedApi();

    if (mBufferDirty || mBufferRefCount <= 1 || contentDirty)
        hwcl->mtkData.mflags |= HWC_DIRTY_LAYER;
    else
        hwcl->mtkData.mflags &= ~HWC_DIRTY_LAYER;

    hwcl->mtkData.layerFlags = mDrawingState.flags;
    hwcl->mtkData.s3dFlags = mDrawingState.s3d_flags;
    // ******************************************************************** //
}
...
void Layer::onDraw(const Region& clip) const
{
    ...
    if (CC_UNLIKELY(mActiveBuffer == 0)) {
        ...
        if (!holes.isEmpty()) {
            XLOGI("[%s] %s(i:%d): clear screen hole (l:%d, t:%d, w:%d, h:%d)",
                ...
                holes.getBounds().left, holes.getBounds().top, holes.getBounds().width(), holes.getBounds().height());
    ...
    if (!isProtected()) {
        ...
        const bool useFiltering = getFiltering() || needsFiltering() || isFixedSize() ||
                                  isStereo3D();  // [mtk03712] for S3D rendering
        ...
        // [mtk03712] buffer conversion here for SF layer
        //--------------------------------------------------------------------
...
void Layer::lockPageFlip(bool& recomputeVisibleRegions)
{
    ...
    if (mQueuedFrames > 0) {
        ...
        ATRACE_INT("updateTexImage", getIdentity());
        if (mSurfaceTexture->updateTexImage(&r) < NO_ERROR) {
            // something happened!
            recomputeVisibleRegions = true;
            ATRACE_INT("updateTexImage", -1);
            return;
        }
        ATRACE_INT("updateTexImage", 0);
        ...
        // [mtk03712] for S3D mode flags
        checkRequestedFlags();
    }

    // [mtk03712] for S3D mode buffer usage info
    updateFlags();
}
        ...
        const Transform& planeTransform, Region& outDirtyRegion)
{
    ...
    mBufferDirty = !postedRegion.isEmpty();
    if (!postedRegion.isEmpty()) {
        ...
        // [mtk04189] case 5: increment first buffer count when the texture has been created
        mBufferRefCount++;
    }
}
