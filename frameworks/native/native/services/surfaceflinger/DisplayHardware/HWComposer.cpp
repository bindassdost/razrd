      ...
      mVSyncCount(0), mDebugForceFakeVSync(false)
{
    ...
    if (err == 0) {
        ...
        if (err == 0) {
            ...
            // [MTK] 20120903: add to get accurate fps, take care for type cast
            int refresh_period = 0;
            mHwc->query(mHwc, HWC_VSYNC_PERIOD, &refresh_period);
...
HWComposer::~HWComposer() {
    ...
    //free(mList);
    // [MTK] 20120815: free member to avoid memory leak
    freeWorkList();
...
status_t HWComposer::prepare() const {
    ...
    // *** MediaTek ******************************************************* //
    if (mList) {
        if (0 != LayerScreenshot::getCount()) {
            mList->mtkListData.mflags |= HWC_LAYERSCREENSHOT_EXIST;
        }

        // Stereo3D related
        mList->mtkListData.s3dOrientation = mFlinger->mDrawingState.composingOrientation;
        mList->mtkListData.s3dPhase       = mFlinger->mDrawingState.composingPhase;

        // External Display related
        mList->mtkListData.extOrientation = mFlinger->graphicPlane(0).getOrientation();
    }
    // ******************************************************************** //
...
status_t HWComposer::commit() const {
    // [mtk04189] get layer swap information and raise the
    //            hwc_layer_list_t flag if necessary
    ...
    if (mList) {
        ...
        // [mtk04189] clear additional flags
...
void HWComposer::dump(String8& result, char* buffer, size_t SIZE,
    ...
    if (mHwc && mList) {
        ...
        for (size_t i=0 ; i<mList->numHwLayers ; i++) {
            ...
            result.appendFormat(
                    " %8s | %4s | %08x | %08x | %08x | %02x | %05x | %08x | [%5d,%5d,%5d,%5d] | [%5d,%5d,%5d,%5d] | %s\n",
    ...
    // [MTK] 20120814: add property function for debug purpose
    if (mVSyncThread != NULL) {
