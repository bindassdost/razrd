...
#warning "using EGL_IMG_context_priority"
    ...
    // [MTK] 20120903: add to get accurate fps
    nsecs_t period;
    mHwc->getRefreshPeriod(&period);
    if (period != mRefreshPeriod) {
        mRefreshPeriod = period;
        mRefreshRate = float(1e9 / mRefreshPeriod);
        ALOGD("Use accurate FPS(%f, %d)", mRefreshRate, mRefreshPeriod);
    } else {
        ALOGD("Use default FPS(%f, %d)", mRefreshRate, mRefreshPeriod);
    }
}
...
{
    // [MTK] 20120807: mHwc should be deleted to avoid memory leak
    delete mHwc;
